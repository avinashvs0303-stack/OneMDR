...
export class IntegrationsService {
  ...
  async runDetectionSearch(
    ...
    const tenantId = actor.tenantId!;
    await this.assertOwnership(tenantId, integrationId);
    ...
    if (!dep) throw new NotFoundException('Detection is not deployed to this integration');
    ...
    if (!sid) throw new BadRequestException('Splunk did not return a search job SID');

    // Poll for completion (max 30 seconds, 1-second intervals)
