...
> = {
  SPLUNK: {
    ...
    fields: [
      {
        key: 'apiToken',
        label: 'API Token (Bearer — for connection test)',
        type: 'password',
        required: false,
      ...
      {
        key: 'username',
        label: 'Username (required for deploy)',
        required: false,
        placeholder: 'admin',
      },
      {
        key: 'password',
        label: 'Password (required for deploy)',
        type: 'password',
        required: false,
        placeholder: '••••••••',
      },
      { key: 'port', label: 'Management Port (leave blank for Cloud)', placeholder: '8089' },
    ],
