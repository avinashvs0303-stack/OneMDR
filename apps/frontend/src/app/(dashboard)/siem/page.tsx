...
const SIEM_INTEGRATIONS = [
  {
    ...
    version: '8.2.6',
    host: 'splunk.demo-corp.internal:8089',
    status: 'connected' as const,
    lastSync: '2 minutes ago',
    latency: 48,
    detectionsDeployed: DETECTIONS.filter((d) => d.siem === 'Splunk' && d.status === 'Active')
      .length,
    alertsToday: 28,
  ...
  },
  {
    ...
    platform: 'Microsoft Sentinel' as const,
    version: 'Cloud',
    host: 'demo-corp.sentinel.azure.com',
    status: 'connected' as const,
    lastSync: '8 minutes ago',
    latency: 61,
    detectionsDeployed: DETECTIONS.filter(
      (d) => d.siem === 'Microsoft Sentinel' && d.status === 'Active',
    ).length,
    alertsToday: 14,
  ...
  },
  {
    ...
    platform: 'Chronicle' as const,
    version: 'Cloud',
    host: 'demo-corp.chronicle.security',
    status: 'warning' as const,
    lastSync: '41 minutes ago',
    latency: 0,
    detectionsDeployed: DETECTIONS.filter((d) => d.siem === 'Chronicle' && d.status === 'Active')
      .length,
    alertsToday: 6,
  ...
  },
  {
    ...
    version: '8.13',
    host: 'demo-corp.es.io:9243',
    status: 'connected' as const,
    lastSync: '5 minutes ago',
    latency: 33,
    detectionsDeployed: DETECTIONS.filter((d) => d.siem === 'Elastic' && d.status === 'Active')
      .length,
    alertsToday: 9,
    ...
    <div className="flex flex-1 flex-col min-h-0 overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
        <div className="grid grid-cols-4 gap-4">
          {[
            {
              label: 'Connected SIEMs',
              value: `${SIEM_INTEGRATIONS.filter((s) => s.status === 'connected').length}/4`,
              color: 'text-emerald-600 dark:text-emerald-400',
            },
            {
              label: 'Detections Deployed',
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          {SIEM_INTEGRATIONS.map((integration) => (
            <div
              key={integration.id}
              className={cn(
                'rounded-xl border backdrop-blur-md p-5 shadow-sm space-y-4',
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className={cn('font-semibold text-sm', integration.accentColor)}>
                      {integration.name}
                    </p>
              ...
              <div className="grid grid-cols-3 gap-3 text-center">
                {[
                  ...
                  {
                    label: 'Latency',
                    value: integration.latency > 0 ? `${integration.latency}ms` : 'Offline',
                  },
                ].map((m) => (
                  <div
                    key={m.label}
                    className="rounded-lg bg-black/5 dark:bg-black/20 border border-black/10 dark:border-white/10 p-2.5"
                  >
              ...
              <div>
                <p className="text-[10px] font-bold uppercase text-slate-400 dark:text-zinc-500 mb-1.5">
                  Monitored Sources
                </p>
              ...
              <div className="flex items-center justify-between pt-1">
                ...
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    className="flex items-center gap-1 rounded px-2.5 py-1 text-xs text-slate-500 dark:text-zinc-400 hover:bg-black/10 dark:hover:bg-white/10 hover:text-slate-900 dark:hover:text-white transition-colors"
                  >
          ...
          <div className="rounded-xl border border-dashed border-black/10 dark:border-white/10 p-5 flex flex-col items-center justify-center gap-3 text-center min-h-[200px]">
            ...
            <p className="text-sm font-medium text-slate-700 dark:text-zinc-300">
              Connect another SIEM
            </p>
            <p className="text-xs text-slate-400 dark:text-zinc-500 max-w-xs">
              Supports QRadar, Sumo Logic, Datadog SIEM, Devo, Securonix, and more.
            </p>
            <button
              type="button"
          ...
          <div className="rounded-xl border border-amber-500/25 bg-amber-50 dark:bg-amber-500/5 backdrop-blur-md p-5 shadow-sm space-y-4">
            ...
            <div className="space-y-2">
              {PENDING.map((det) => (
                <div
                  key={det.id}
                  className="flex items-center justify-between rounded-lg border border-black/10 dark:border-white/10 bg-white/60 dark:bg-black/20 px-4 py-3"
                >
                  <div>
                    ...
                    <p className="text-xs text-slate-400 dark:text-zinc-500">
                      {det.techniqueId} · {det.siem} · {det.queryLanguage}
                    </p>
                  </div>
                  <button
                    type="button"
                    className="shrink-0 rounded-lg bg-amber-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-amber-500 transition-colors"
                  >
                    Deploy to {det.siem}
                  </button>
                </div>
