      ...
      <main className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
        <div className="grid grid-cols-2 gap-4 xl:grid-cols-4">
          {METRICS.map((m) => (
            <div
              key={m.label}
              className="rounded-xl border border-black/10 dark:border-white/10 bg-white/70 dark:bg-white/5 backdrop-blur-md p-5 shadow-sm dark:shadow-lg"
            >
            ...
            <div className="flex items-end gap-2 h-32">
              {VELOCITY.map((v) => (
                <div key={v.week} className="flex flex-1 flex-col items-center gap-1">
                  ...
                  <div
                    className="w-full rounded-t-md bg-amber-500/15 hover:bg-amber-500/30 transition-colors relative overflow-hidden"
                    style={{ height: `${(v.completed / MAX_VELOCITY) * 100}%` }}
                  >
                    <div
                      className="absolute inset-x-0 bottom-0 rounded-t-md bg-amber-500"
                      style={{ height: `${(v.completed / MAX_VELOCITY) * 60}%` }}
                    />
                  </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3 text-[10px] text-slate-500 dark:text-zinc-400">
                <span className="flex items-center gap-1">
                  <span className="h-2 w-2 rounded-sm bg-emerald-500" /> On track
                </span>
                <span className="flex items-center gap-1">
                  <span className="h-2 w-2 rounded-sm bg-amber-400" /> At risk
                </span>
                <span className="flex items-center gap-1">
                ...
                <div key={b.name} className="space-y-1.5">
                  ...
                  <div className="flex h-2 w-full overflow-hidden rounded-full bg-black/10 dark:bg-white/10">
                    <div
                      ...
                      style={{ width: `${(b.onTrack / b.total) * 100}%` }}
                    />
                    <div
                      className="bg-amber-400"
                      style={{ width: `${(b.atRisk / b.total) * 100}%` }}
                    />
                    <div
                      className="bg-red-500"
                      style={{ width: `${(b.overdue / b.total) * 100}%` }}
                    />
                  </div>
                </div>
              ))}
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {['Alice Owner', 'Bob Admin', 'Carol Member', 'Dave Guest'].map((name, i) => {
              ...
              return (
                <div
                  key={name}
                  className="rounded-lg border border-black/10 dark:border-white/10 bg-black/5 dark:bg-black/20 p-3 space-y-2"
                >
                  <div className="flex items-center gap-2">
                    <div className="h-7 w-7 rounded-full bg-amber-600 flex items-center justify-center text-[10px] font-bold text-white">
                      {name
                        .split(' ')
                        .map((n) => n[0])
                        .join('')}
