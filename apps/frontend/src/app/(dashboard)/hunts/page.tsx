...
const HUNTS = [
  {
    id: 'HNT-0006',
    title: 'Kerberoasting & Pass-the-Hash Campaign',
    hypothesis:
      'Threat actors may be using Kerberoasting to crack service account hashes and subsequently leveraging PtH for lateral movement across the Active Directory environment.',
  ...
  },
  {
    ...
    hypothesis:
      'Adversaries may be abusing signed Windows binaries (LOLBins) including certutil, mshta, and regsvr32 to proxy malicious payload execution and evade endpoint defences.',
    status: 'Complete' as HuntStatus,
    analyst: 'Bob Admin',
    tacticId: 'TA0005',
    tactic: 'Defense Evasion',
    techniques: ['T1218.001', 'T1218.005', 'T1218.010'],
  ...
  {
    id: 'HNT-0004',
    title: 'Ransomware Pre-Deployment Indicators',
    hypothesis:
      'Ransomware operators typically perform recon, disable backups, and exfiltrate data 24–72h before encryption. Hunt for shadow copy deletion, volume enumeration, and large archive creation.',
    status: 'Complete' as HuntStatus,
    analyst: 'Carol Member',
    tacticId: 'TA0040',
    tactic: 'Impact',
    techniques: ['T1490', 'T1486', 'T1048'],
  ...
  {
    id: 'HNT-0003',
    title: 'Cloud Identity Persistence via OAuth App Grants',
    hypothesis:
      'Threat actors compromise OAuth apps with excessive permissions to maintain persistence in Microsoft 365 and Azure AD tenants, surviving password resets and MFA enforcement.',
    status: 'Planned' as HuntStatus,
    analyst: 'Alice Owner',
    tacticId: 'TA0003',
    tactic: 'Persistence',
  ...
  {
    id: 'HNT-0002',
    title: 'C2 Beaconing via Encrypted DNS',
    hypothesis:
      'Threat actors increasingly use DNS-over-HTTPS (DoH) to tunnel C2 communications through encrypted DNS, bypassing traditional DNS-based detection.',
    status: 'Complete' as HuntStatus,
    analyst: 'Bob Admin',
    tacticId: 'TA0011',
    tactic: 'Command and Control',
    techniques: ['T1071.004', 'T1573'],
...
const STATUS_CONFIG: Record<HuntStatus, { dot: string; badge: string }> = {
  Active: {
    ...
    badge: 'text-emerald-600 dark:text-emerald-400 bg-emerald-500/20 border-emerald-500/30',
  },
  Planned: {
    dot: 'bg-amber-500',
    badge: 'text-amber-600 dark:text-amber-400 bg-amber-500/20 border-amber-500/30',
  },
  Complete: {
    dot: 'bg-slate-400 dark:bg-zinc-500',
    badge:
      'text-slate-500 dark:text-zinc-400 bg-black/5 dark:bg-white/5 border-black/10 dark:border-white/15',
  },
  Archived: {
...
const PRIORITY_COLORS: Record<string, string> = {
  Critical:
    'text-red-600 dark:text-red-400 bg-red-500/20 border border-red-500/30 shadow-[0_0_8px_rgba(239,68,68,0.08)]',
        ...
        <div className="space-y-3">
          {HUNTS.map((hunt) => {
            ...
            return (
              <div
                key={hunt.id}
                className="rounded-xl border border-black/10 dark:border-white/10 bg-white/70 dark:bg-white/5 backdrop-blur-md p-5 shadow-sm dark:shadow-lg hover:bg-white/90 dark:hover:bg-white/10 hover:border-black/20 dark:hover:border-white/20 transition-all cursor-pointer group"
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1.5 text-xs text-slate-500 dark:text-zinc-400 leading-relaxed line-clamp-2">
                        {hunt.hypothesis}
                      </p>
                    </div>
                  </div>
