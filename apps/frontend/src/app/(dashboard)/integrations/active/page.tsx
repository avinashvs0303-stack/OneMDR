...
export default function ActiveIntegrationsPage() {
  ...
  return (
    <div className="flex flex-1 flex-col min-h-0">
      ...
      <div className="flex items-center justify-between border-b border-black/10 dark:border-white/10 bg-black/5 dark:bg-black/20 backdrop-blur-md px-6 py-3 gap-4 shrink-0">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg border border-black/10 dark:border-white/10 bg-white/70 dark:bg-white/5 px-3 py-1.5 text-xs font-medium text-slate-600 dark:text-zinc-300 hover:bg-black/5 dark:hover:bg-white/10 transition-colors"
          >
            <Grid2X2 className="h-3.5 w-3.5" /> Browse Apps
          </button>
          <button
            type="button"
            onClick={() => setShowAddModal(true)}
            className="flex items-center gap-1.5 rounded-lg bg-teal-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-teal-500 transition-colors shadow-sm"
          >
      ...
      {/* Info banner */}
      <div className="flex items-center gap-2 px-6 py-2 bg-teal-500/10 border-b border-teal-500/20 text-[11px] text-teal-700 dark:text-teal-300">
        <Plug className="h-3.5 w-3.5 shrink-0" />
        <span>
          Connect your SIEM platforms to deploy detection rules directly from the Detection Library.
          All credentials are stored encrypted server-side and never transmitted to third parties.
        </span>
      </div>
      ...
      <div className="flex-1 overflow-auto p-6 space-y-8">
        {loading ? (
          ...
          <EmptyState onBrowse={() => router.push('/integrations/browse')} />
        ) : (
    ...
    >
      ...
      <div className="flex items-center gap-1 px-3 py-2">
        <button
          ...
          onClick={onTest}
          disabled={isTesting}
          className="flex items-center gap-1 rounded-lg border border-black/10 dark:border-white/10 bg-white/70 dark:bg-white/5 px-2.5 py-1.5 text-[11px] font-medium text-slate-600 dark:text-zinc-300 hover:bg-black/5 dark:hover:bg-white/10 transition-colors disabled:opacity-50"
        >
          {isTesting ? (
        ...
        <button
          ...
          className={cn(
            ...
            integration.isEnabled
              ? 'border-black/10 dark:border-white/10 bg-white/70 dark:bg-white/5 text-slate-600 dark:text-zinc-300'
            ...
            <div className="space-y-3">
              <Field label="Integration Name" required>
                <input
                  ...
                  className="w-full rounded-lg border border-black/10 dark:border-white/10 bg-white dark:bg-white/5 px-3 py-1.5 text-xs text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-500/50"
                />
              </Field>
                  ...
                  <Field key={field.key} label={field.label} required={field.required}>
                    <div className="relative">
                      <input
                        ...
                        className="w-full rounded-lg border border-black/10 dark:border-white/10 bg-white dark:bg-white/5 px-3 py-1.5 pr-8 text-xs text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-500/50"
                      />
                      {field.type === 'password' && (
                        <button
                          type="button"
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
      <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-teal-500/10 border border-teal-500/20">
        <Plug className="h-7 w-7 text-teal-500 dark:text-teal-400" />
      </div>
      <div>
        ...
        <p className="mt-1 text-xs text-slate-400 dark:text-zinc-500">
          Browse available apps and connect your first SIEM or security tool.
        </p>
      </div>
      <button
        type="button"
...
const LEVEL_STYLE: Record<LogLevel, { badge: string; row: string }> = {
  INFO: { badge: 'bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20', row: '' },
  WARN: {
