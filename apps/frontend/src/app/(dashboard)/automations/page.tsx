...
const AUTOMATIONS = [
  {
    id: '1',
    name: 'Move to Done when all sub-items complete',
    board: 'Sprint Planning',
    trigger: 'Sub-item completed',
  ...
  {
    id: '2',
    name: 'Assign due date to new items',
    board: 'Bug Tracker',
    trigger: 'Item created',
    action: 'Set due date +3 days',
    runs: 22,
    enabled: true,
    lastRun: '1h ago',
  },
  {
    id: '3',
    name: 'Notify owner when item moves to Review',
  ...
  {
    id: '4',
    name: 'Archive items older than 90 days',
    board: 'All boards',
  ...
  {
    id: '5',
    name: 'Flag overdue items in red',
    board: 'Marketing Q3',
    ...
    <div className="flex flex-1 flex-col min-h-0 overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            type="button"
            className="flex items-center gap-1.5 rounded-lg bg-amber-600 px-4 py-2 text-sm font-semibold text-white hover:bg-amber-500 transition-colors shadow-sm shadow-amber-500/20"
          >
        ...
        <div className="rounded-xl border border-black/10 dark:border-white/10 bg-white/70 dark:bg-white/5 backdrop-blur-md shadow-sm dark:shadow-lg divide-y divide-black/5 dark:divide-white/5 overflow-hidden">
          {AUTOMATIONS.map((a) => (
            <div
              key={a.id}
              className="flex items-center gap-4 px-5 py-4 hover:bg-black/5 dark:hover:bg-white/5 transition-colors cursor-pointer group"
            >
              <div
                className={cn(
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-0.5 flex items-center gap-3 text-[11px] text-slate-500 dark:text-zinc-400">
                  <span className="flex items-center gap-1">
                    <Play className="h-3 w-3" /> {a.trigger}
                  </span>
                  <span>→ {a.action}</span>
                  <span className="hidden sm:flex items-center gap-1">
                    <Clock className="h-3 w-3" /> {a.lastRun}
                  </span>
                  <span className="text-amber-600 dark:text-amber-400 font-medium">{a.board}</span>
                </div>
              </div>
        ...
        <div className="rounded-xl border border-dashed border-black/10 dark:border-white/10 p-8 text-center">
          ...
          <p className="mt-1 text-xs text-slate-500 dark:text-zinc-400">
            Create rules that trigger actions automatically — no code required.
          </p>
          <button
            type="button"
            className="mt-4 inline-flex items-center gap-1.5 rounded-lg bg-amber-600 px-4 py-2 text-sm font-semibold text-white hover:bg-amber-500 transition-colors shadow-sm shadow-amber-500/20"
          >
