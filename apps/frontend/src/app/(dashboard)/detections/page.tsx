          ...
          <div className="flex-1 overflow-y-auto">
            <table className="w-full text-sm">
              <thead className="sticky top-0 z-10 bg-white/90 dark:bg-black/40 backdrop-blur-md border-b border-black/10 dark:border-white/10">
                <tr>
                  {['', 'Detection', 'Technique', 'SIEM', 'Severity', 'FP%', 'Alerts/d'].map(
                    (h) => (
                      <th
                        key={h}
                ...
                {filtered.map((det) => (
                  ...
                  >
                    <td className="px-3 py-3">
                      <ChevronRight
                        className={cn(
                          ...
                          selected?.id === det.id && 'rotate-90 text-amber-500 dark:text-amber-400',
                        )}
                      />
                    </td>
                    ...
                    <td className="px-3 py-3">
                      ...
                      <p className="text-[10px] text-slate-400 dark:text-zinc-500 truncate max-w-[120px]">
                        {det.tactic}
                      </p>
                    </td>
                    <td className="px-3 py-3">
          ...
          <div className="w-[45%] flex flex-col overflow-hidden bg-black/5 dark:bg-black/20 backdrop-blur-md">
            <div className="border-b border-black/10 dark:border-white/10 px-5 py-4 space-y-3">
              <div className="flex items-start justify-between gap-3">
                ...
                <button
                  ...
                  className="text-slate-400 dark:text-zinc-500 hover:text-slate-900 dark:hover:text-white shrink-0 text-lg leading-none"
                >
                  ×
                </button>
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
              ...
              {selected.status !== 'Draft' && (
                <div className="grid grid-cols-3 gap-3">
                  {[
                    {
                      label: 'Alerts/Day',
                      value: selected.metrics.alertsPerDay,
                      color: 'text-slate-900 dark:text-white',
                    },
                    {
                      label: 'FP Rate',
                      value: `${selected.metrics.fpRate}%`,
                      color:
                        selected.metrics.fpRate > 15
                          ? 'text-red-500 dark:text-red-400'
                          : selected.metrics.fpRate > 8
                            ? 'text-amber-500 dark:text-amber-400'
                            : 'text-emerald-600 dark:text-emerald-400',
                    },
                    {
                      label: 'MTTD',
                      value: `${selected.metrics.mttd}h`,
              ...
              <div className="rounded-lg border border-black/10 dark:border-white/10 bg-white/60 dark:bg-white/5 p-3 space-y-1.5">
                ...
                <p className="text-xs font-medium text-slate-900 dark:text-white">
                  {selected.tactic} ·{' '}
                  <span className="text-amber-600 dark:text-amber-400">{selected.techniqueId}</span>
                </p>
              ...
              <div className="flex gap-4 text-[10px] text-slate-400 dark:text-zinc-500 border-t border-black/10 dark:border-white/10 pt-3">
                <span className="flex items-center gap-1">
                  <Clock className="h-3 w-3" /> Created {selected.created}
                </span>
                <span className="flex items-center gap-1">
            ...
            {selected.status !== 'Retired' && (
              <div className="border-t border-black/10 dark:border-white/10 px-5 py-3">
                <button
                  type="button"
                  className="flex w-full items-center justify-center gap-2 rounded-lg bg-amber-600 py-2.5 text-sm font-semibold text-white hover:bg-amber-500 transition-colors shadow-sm shadow-amber-500/20"
                >
