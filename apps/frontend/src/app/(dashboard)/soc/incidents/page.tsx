...
import { Plus, X, Clock, ChevronDown, Filter } from 'lucide-react';
...
const SEV_CONFIG: Record<
  Severity,
  { label: string; color: string; badgeClass: string; responseSec: number; resolutionSec: number }
> = {
  P1: {
    label: 'Critical',
...
const SEED_INCIDENTS: Incident[] = [
  {
    ...
    description:
      'Volume shadow copy deletion observed followed by mass file extension change. Affected host isolated.',
  ...
  },
  {
    ...
    description: 'PsExec service installation detected on PROD-APP-03. Source host quarantined.',
    status: 'NEW',
    assignee: null,
  ...
  },
  {
    ...
    ref: 'INC-0003',
    severity: 'P1',
    title: 'LSASS credential dump — CORP-DC-02',
    description: 'Non-system process accessed LSASS memory. Mimikatz-like behaviour observed.',
    status: 'RESOLVED',
  ...
  },
  {
    ...
    ref: 'INC-0004',
    severity: 'P3',
    title: 'Suspicious scheduled task creation on WORKSTATION-22',
    description:
      'Scheduled task pointing to temp directory binary. Persistence mechanism suspected.',
    status: 'MITIGATED',
    assignee: 'Avinash VS',
...
function SlaTimer({
  ...
  useEffect(() => {
    ...
    const resolutionMs = SEV_CONFIG[severity].resolutionSec * 1000;
...
function CreateModal({
  ...
  onClose: () => void;
  onCreate: (inc: Partial<Incident>) => void;
}) {
...
export default function IncidentsPage() {
  ...
  const advance = (id: string) => {
    setIncidents((prev) =>
      prev.map((i) => {
        if (i.id !== id) return i;
        const next = STATUS_CONFIG[i.status].next;
        if (!next) return i;
        return {
          ...i,
          status: next,
  ...
  const create = (data: Partial<Incident>) => {
    const newInc: Incident = {
      id: String(incidents.length + 1),
      ref: `INC-${String(counter).padStart(4, '0')}`,
      severity: data.severity ?? 'P2',
      title: data.title ?? '',
      description: data.description ?? '',
      status: 'NEW',
      assignee: data.assignee ?? user?.firstName ?? null,
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            <div key={label} className={cn('rounded-xl border p-4 space-y-1', bg)}>
              ...
              <p className={cn('text-3xl font-black', cls)}>{value}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-1.5 ml-auto">
                ...
                className={cn(
                  ...
                  filterSev === s
                    ? s === 'ALL'
                      ...
                      : (SEV_CONFIG[s as Severity]?.badgeClass ?? '')
            ...
            {(['ALL', 'NEW', 'INVESTIGATING', 'MITIGATED', 'RESOLVED'] as const).map((s) => (
              ...
              >
                {s === 'ALL' ? 'All Status' : STATUS_CONFIG[s as IncidentStatus].label}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="rounded-xl border border-black/10 dark:border-white/10 bg-white/70 dark:bg-white/5 backdrop-blur-md shadow-sm dark:shadow-lg overflow-hidden">
          ...
          <div className="divide-y divide-black/5 dark:divide-white/5">
            ...
            {filtered.map((inc) => {
              ...
              const stc = STATUS_CONFIG[inc.status];
              return (
                <div
                  key={inc.id}
                  className={cn(
