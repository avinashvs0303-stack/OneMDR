...
-- FK to tenants intentionally omitted — tenant isolation enforced in application layer
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'set_soc_documents_updated_at') THEN
    CREATE TRIGGER set_soc_documents_updated_at
      BEFORE UPDATE ON soc_documents FOR EACH ROW EXECUTE FUNCTION set_updated_at();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'set_soc_changes_updated_at') THEN
    CREATE TRIGGER set_soc_changes_updated_at
      BEFORE UPDATE ON soc_changes FOR EACH ROW EXECUTE FUNCTION set_updated_at();
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'set_soc_requests_updated_at') THEN
    CREATE TRIGGER set_soc_requests_updated_at
      BEFORE UPDATE ON soc_service_requests FOR EACH ROW EXECUTE FUNCTION set_updated_at();
  END IF;
END $$;
